...
NativeCamera::NativeCamera(struct android_app * app) :
                  androidApp_(app),
                  cameraMgr_(nullptr),
                  activeCameraId_(""),
    ...
    ANativeWindow_acquire(androidApp_->window);
    CALL_CONTAINER(create(&outputContainer_));
    CALL_OUTPUT(create(androidApp_->window, &output_));
    CALL_CONTAINER(create(&outputContainer_));
    CALL_CONTAINER(add(outputContainer_, output_));

    // Create output target from the same ANativeWindow, add it into captureRequest
    CALL_TARGET(create(androidApp_->window, &outputTarget_));
...
NativeCamera::~NativeCamera() {
    ...
    ANativeWindow_release(androidApp_->window);
